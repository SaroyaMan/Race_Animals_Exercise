...
Tile* Frog::lookAhead(Tile*** map, Point2D end_p)
{
...
int Frog::cheap(Tile* t1, Tile* t2)
{
	float v1=0, v2=0;
	if (t1->getType() == easyTile) v1=2;
	if (t1->getType() == normalTile) v1=1;
	if (t1->getType() == cropTile) v1=1;
	if (t1->getType() == waterTile) v1=0.5;
	
	if (t2->getType() == easyTile) v2=2;
	if (t2->getType() == normalTile) v2=1;
	if (t2->getType() == cropTile) v2=1;
	if (t2->getType() == waterTile) v2=0.5;	
...
{
	os<<"Name of the competitor: "<<f.getName()<<endl;
	os<<"Total movement cost of the competitor: "<<f.getTotalMovementCost()<<endl;
	os<<"Current tile of the competitor: ";
	f.getCurrentTile()->whoAmI();
	os<<f.getCurrentTile()->getPosition();
}
